...
/**
 * NMS block scanner for Minecraft 1.13-1.16 using Spigot mappings with version suffixes.
 * Package structure: net.minecraft.server.v1_16_R3.*
 */
public class SpigotNmsScanner extends AbstractBlockScanner {
    
    private final boolean initialized;
    private final String versionSuffix;
    
    // NMS classes (Spigot-mapped with version suffix)
    ...
    public SpigotNmsScanner(Plugin plugin, PluginConfig config, CounterDataManager counterManager) {
    ...
    private boolean initialize() {
        if (versionSuffix == null || versionSuffix.isEmpty()) {
            CSLLogger.debug(() -> "[SpigotNMS] No version suffix detected");
            return false;
        }

        try {
            CSLLogger.debug(() -> "[SpigotNMS] Detected version suffix: " + versionSuffix);
            ...
            // NMS classes with version suffix
            worldServerClass = loadClass("net.minecraft.server." + versionSuffix + ".WorldServer");
            if (worldServerClass == null) {
                worldServerClass = loadClass("net.minecraft.server." + versionSuffix + ".World");
            }
            ...
            if (worldServerClass == null || blockPositionClass == null || iBlockDataClass == null) {
                CSLLogger.debug(() -> "[SpigotNMS] Failed to load required NMS classes");
                return false;
            }
            ...
            try {
                ...
                CSLLogger.debug(() -> "[SpigotNMS] BlockPosition(int,int,int) constructor not found");
                return false;
            }

            // World.getType(BlockPosition) method
            getType = findMethod(worldServerClass, blockPositionClass, "getType", "a");
            if (getType == null) {
                CSLLogger.debug(() -> "[SpigotNMS] getType method not found");
                return false;
            }
            ...
            try {
                ...
                try {
                    ...
                    CSLLogger.debug(() -> "[SpigotNMS] getBlock method not found");
                    getBlock = null;
                }
            }
            ...
            try {
                ...
                CSLLogger.debug(() -> "[SpigotNMS] CraftMagicNumbers.getMaterial not found");
                craftMagicNumbers_getMaterial = null;
            }

            CSLLogger.debug(() -> "[SpigotNMS] Initialization successful for " + versionSuffix);
            return true;

        } catch (Exception e) {
            Bukkit.getLogger().warning("[SpigotNMS] Initialization failed: " + e.getMessage());
            return false;
        }
    }
    ...
    protected Material getMaterialAtImpl(World world, int x, int y, int z) {
        ...
        try {
            // Get NMS WorldServer from CraftWorld
            Object worldServer = craftWorld_getHandle.invoke(world);
            if (worldServer == null) {
                return null;
            }
            ...
            // Get IBlockData
            Object iBlockData = getType.invoke(worldServer, blockPosition);
            if (iBlockData == null) {
                return null;
            }
            ...
            if (getBlock != null && craftMagicNumbers_getMaterial != null) {
                try {
                    ...
                    CSLLogger.debug(() -> "[SpigotNMS] Material conversion failed: " + e.getMessage());
                }
            }
            ...
            CSLLogger.debug(() -> "[SpigotNMS] Error getting material: " + e.getMessage());
            return null;
        }
    }
    ...
    public String getImplementationName() {
        return "SpigotNMS" + (versionSuffix != null ? " (" + versionSuffix + ")" : "");
    }
