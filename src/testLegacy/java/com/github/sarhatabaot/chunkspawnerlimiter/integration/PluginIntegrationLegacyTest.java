 ...
 */
@DisplayName("Plugin Integration Tests (Legacy)")
class PluginIntegrationLegacyTest {
    ...
    @BeforeEach
    void setUp() {
        // Create a mock server with legacy MockBukkit
    ...
    @Test
    @DisplayName("Should load plugin successfully with legacy MockBukkit")
    ...
    @Test
    @DisplayName("Should handle legacy Minecraft version compatibility")
    void shouldHandleLegacyMinecraftVersionCompatibility() {
        // Given - Plugin is loaded

        // When - Enable plugin
        plugin.onEnable();

        // Then - Plugin should function with legacy MockBukkit
        // This test ensures the plugin can initialize and run with older Minecraft versions
        assertThat(plugin.isEnabled()).isTrue();

        // Test basic functionality works
        PluginConfig config = plugin.getPluginConfig();
        assertThat(config).isNotNull();
        assertThat(config.getRemovalMode()).isNotNull();
    }
}
